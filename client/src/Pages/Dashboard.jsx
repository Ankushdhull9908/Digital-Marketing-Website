...
function Badge({ text, color }) {
  ...
  return (
    <span
      className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide ${map[color] || map.slate}`}
    >
      {text}
    </span>
  );
...
function Modal({ title, onClose, onSave, children, wide }) {
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div
        className={`bg-white rounded-2xl w-full ${wide ? "max-w-3xl" : "max-w-lg"} shadow-2xl border border-slate-100 mx-4`}
      >
...
function ResumeDetailModal({ resume, onClose }) {
  ...
  const row = (label, value) =>
    value ? (
      <div className="flex gap-2 text-sm">
        <span className="w-28 flex-shrink-0 text-slate-400 font-medium">
          {label}
        </span>
  ...
  return (
    <Modal
      title={`Resume — ${resume.name || "Unnamed"}`}
      onClose={onClose}
      wide
    >
      {/* Header row */}
      ...
      <div className="space-y-2">
        ...
        {row(
          "LinkedIn",
          resume.linkedin && `linkedin.com/in/${resume.linkedin}`,
        )}
      </div>
            ...
            <div key={i} className="mb-3 pl-3 border-l-2 border-slate-200">
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {p.tags?.map((t, j) => (
                  <span
                    key={j}
                    className="px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded text-[10px] font-semibold"
                  >
                    {t}
                  </span>
                ))}
      ...
      <div className="pt-2 border-t border-slate-100 flex flex-wrap gap-4 text-xs text-slate-400">
        <span>
          Session: <span className="font-mono">{resume.sessionId}</span>
        </span>
        <span>
          Last edited: {new Date(resume.lastEditedAt).toLocaleString("en-IN")}
        </span>
        <span>
          Created: {new Date(resume.createdAt).toLocaleString("en-IN")}
        </span>
      </div>
    </Modal>
  );
...
export function MyApplicationsPanel() {
  ...
  useEffect(() => {
    get("/jobs/user/my-applications").then((d) => {
            ...
            {apps.map((app) => (
              <tr key={app._id} className="hover:bg-slate-50">
                ...
                <td className="px-5 py-4">
                  <Badge
                    text={app.status}
                    color={STATUS_COLOR[app.status] ?? "slate"}
                  />
                </td>
                <td className="px-5 py-4">
                  {app.resumeUrl ? (
...
function StatCard({ label, value, sub, icon, color }) {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group">
      <div
        className={`absolute top-3 right-3 opacity-10 group-hover:opacity-20 transition-opacity ${colors[color]}`}
      >
        {icon}
      </div>
...
function Dashboard() {
  ...
  const tabMeta = {
    ...
    homepage: {
      ...
      sub: "Manage slider images, project gallery, and client testimonial videos.",
    },
    resumes: {
      title: "User Resumes",
      sub: "View, search, and manage all resumes created by users.",
    },
    faqs: { title: "FAQs", sub: "Manage homepage FAQ section." },
    clients: {
      title: "Clients",
      sub: "Add and manage client logos on the homepage.",
    },
    packages: {
      title: "Packages",
      sub: "Control pricing plans shown on the homepage.",
    },
    testimonials: {
      title: "Testimonials",
      sub: "Manage client testimonials shown on the homepage.",
    },
    contacts: {
      title: "Contacts",
      sub: "View and respond to messages from users.",
    },
    jobs: { title: "Jobs", sub: "Manage job postings and applications." },
    influencer: {
      title: "Influencer Hub",
      sub: "Manage brand campaigns and influencer profiles.",
    },
    analytics: { title: "Analytics", sub: "Site performance overview." },
    blogs: {
      title: "Blog Posts",
      sub: "Create and manage blog articles shown on the website.",
    },
  };
    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <div className="w-16 md:w-64 bg-white border-r border-slate-200 flex flex-col flex-shrink-0 transition-all duration-300">
        ...
        <div className="p-4 md:p-6 flex items-center justify-center md:justify-start gap-3 border-b border-slate-100">
          ...
          <h1 className="hidden md:block text-xl font-bold tracking-tight text-slate-800">
            WebTech
          </h1>
        </div>
          ...
          {nav.map((n) => (
            <React.Fragment key={n.id}>
              <button
                onClick={() => setTab(n.id)}
                title={n.label}
                className={`w-full flex items-center justify-center md:justify-start gap-3 px-3 md:px-4 py-2.5 rounded-xl font-medium transition-all text-sm ${
                  tab === n.id
              ...
              >
                ...
                {n.id === "contacts" && (
                  <span className="hidden md:flex ml-auto w-5 h-5 bg-red-500 text-white text-[10px] rounded-full items-center justify-center font-bold">
                    2
                  </span>
                )}
              </button>
            ...
            <>
              ...
              <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl border border-emerald-100 p-6 mb-6 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    Browse, search, and manage resumes created by users on the
                    Resume Maker.
                  </p>
                </div>
                <button
              ...
              <div className="bg-gradient-to-br from-teal-50 to-blue-50 rounded-2xl border border-teal-100 p-6 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    Update slider images, project gallery, and client
                    testimonial videos — all in one place.
                  </p>
                </div>
                <button
